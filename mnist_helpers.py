...
def show_some_digits(images, targets, sample_size=24, title_text='Digit {}' ):
    ...
    rand = np.random.choice(images.shape[0],nsamples)
    rand_idx = [ images.index.to_list()[i] for i in rand]
    images_and_labels = []
    for i in range(nsamples):
        images_and_labels.append([images.loc[rand_idx[i],:], targets[rand_idx[i]]])
